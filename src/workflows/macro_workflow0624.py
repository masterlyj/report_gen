...
from duckduckgo_search import DDGS  # DuckDuckGo搜索引擎接口
...
from tools.document_processing.doc_converter import convert_to_docx_basic, convert_to_docx_with_indent

# 导入项目自定义模块
from frameworks.pocketflow import Node, Flow  # 工作流节点和流程管理
...
def search_web(term: str):
    ...
    Returns:
        list: 搜索结果列表，每个结果包含标题、链接、摘要等信息
              如果搜索失败则返回空列表
        
    Raises:
        Exception: 当网络请求或搜索过程中出现问题时抛出
    """
    print(f"  > 正在搜索关键词: {term}")
    try:
        with DDGS() as ddgs:
            # 使用中文区域设置，限制返回3个结果以提高效率
            results = ddgs.text(keywords=term, region="cn-zh", max_results=3)
        return list(results)
        ...
        for line in lines:
            ...
            if line.startswith("#### "):
                ...
                if not re.match(r'^\d+\.\d+\.\d+\s+', title_text):
                    new_lines.append(f"#### {section_number}.{len(new_lines)//10 + 1}.{len(new_lines)%10 + 1} {title_text}")
                else:
...
if __name__ == "__main__":
    ...
    # 设置初始共享状态
    shared_state = {
        "topic": "生成式AI基建与算力投资趋势的宏观经济背景（2023-2026）",
